...
// PeerSummary is a snapshot of a peer's state safe to expose externally.
...
func (s *Server) Peers() []PeerSummary {
        s.mu.RLock()
        defer s.mu.RUnlock()
        ...
        for addr, p := range s.peers {
                ...
                summary := PeerSummary{
                        ...
                        UptimeSec:   int64(time.Since(p.connectedAt).Seconds()),
                }
                if p.info != nil {
                        summary.ChainID  = p.info.ChainID
                        summary.Height   = p.info.Height
                        summary.NodeMode = p.info.NodeMode
                        summary.Version  = p.info.Version
                }
                p.mu.Unlock()
