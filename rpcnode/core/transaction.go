...
func NewTransaction(from, to string, value *big.Int, nonce uint64, data []byte) *Transaction {
	...
	tx := &Transaction{
		...
		GasLimit: 21_000, GasPrice: big.NewInt(1_000_000_000), GasUsed: 21_000,
		Nonce: nonce, Data: data, Type: TxTypeTransfer,
...
func (tx *Transaction) computeHash() string {
	raw := fmt.Sprintf("%s:%s:%s:%d:%d:%d", tx.From, tx.To, tx.Value.String(), tx.Nonce, tx.GasPrice.Int64(), tx.Timestamp)
...
func (tx *Transaction) ToMap() map[string]interface{} {
	return map[string]interface{}{
		...
		"value": tx.Value.String(), "gasLimit": tx.GasLimit,
		"gasPrice": tx.GasPrice.String(), "gasUsed": tx.GasUsed,
		"nonce": tx.Nonce, "type": tx.Type, "status": tx.Status,
		"timestamp": tx.Timestamp, "blockNumber": tx.BlockNum,
	}
}
