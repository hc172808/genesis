...
func (p *Peer) Start()             { go p.readLoop(); go p.writeLoop(); go p.pingLoop() }
...
func (s *Server) acceptLoop(ln net.Listener) {
	for {
		...
		s.mu.Unlock()
		peer.Start()
		handshake, _ := json.Marshal(PeerInfo{ChainID: s.chainID, Height: s.height(), NodeMode: "rpc", Version: "1.0.0"})
