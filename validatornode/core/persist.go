...
import (
	...
	"github.com/gydschain/validatornode/storage"
)
...
func (c *Chain) persistBlock(b *Block) {
	...
	for _, addr := range touchedAddresses(b) {
		...
		as := accountStore{Balance: state.Balance.String(), Nonce: state.Nonce}
...
func (c *Chain) loadFromDB() error {
	...
	if loaded > 0 || loadedAccts > 0 {
		log.Info().Int("blocks", loaded).Int("accounts", loadedAccts).Uint64("height", c.Height()).Msg("Chain state restored from LevelDB")
	}
	return nil
}
